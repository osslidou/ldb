...
module.exports = function (hostname, port) {
    ...
    module.put = function (path, postData, expected, timeout) {
        var request = getRequestObject('put', path, timeout, postData);
        return runAndCheck( request, expected);
    }

    module.post = function ( path, postData, expected, timeout) {
        var request = getRequestObject('post', path, timeout, postData);
        return runAndCheck( request, expected);
    }

    module.get = function ( path, expected, timeout) {
        var request = getRequestObject('get', path, timeout);
        return runAndCheck( request, expected);
    }

    module.del = function ( path, postData, expected, timeout) {
        var request = getRequestObject('delete', path, timeout);
        return runAndCheck( request, expected);
    }
    ...
    runAndCheck = function ( request, expected) {
